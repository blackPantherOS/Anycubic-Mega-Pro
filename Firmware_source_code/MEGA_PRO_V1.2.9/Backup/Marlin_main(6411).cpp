...
void prepare_laser_print()
{
  ...
  if(status == 0)
  {
	...
	//enqueue_and_echo_commands_P(PSTR("G1 Z50"));//运动到制定高度
	enqueue_and_echo_commands_P(PSTR("G1 X100 Y130 F6000"));//运动到制定高度
	status =1;
  }
  else if(status==1)
  {
  ...
  if(laser_on_off==1)
		...
		if(laser_indicate_steps == 0)
	{
		...
		if(Laser_printer_st.pic_y_mirror==1)
		 ...
		 sprintf_P(cvalue,PSTR("G1 X%f Y%f F3000"),x_start,y_start); 
