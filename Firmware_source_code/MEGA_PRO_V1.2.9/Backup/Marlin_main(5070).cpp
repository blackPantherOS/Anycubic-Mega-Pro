...
void Read_BMP(unsigned  char *gray,unsigned int y,unsigned int x)
{
	 ...
	 Y=buffer[2]*0.212671+buffer[1]*0.715160+buffer[0]*0.072169; // Y=0.212671*R + 0.715160*G + 0.072169*B,???????;  Gray = (R*299 + G*587 + B*114 + 500) / 1000
	 *gray=0xff-(unsigned char)Y;
	 ...
	 else if(printer_st.pic_bit==24)
	 {
		...
		printer_st.pic_ptr =(printer_st.pic_hight-y-1)*printer_st.pic_realy_widht+x*3+printer_st.pic_start;  
		card.setIndex( printer_st.pic_ptr);
		card.read_bytes(buffer,3);
	   Y=buffer[2]*0.212671+buffer[1]*0.715160+buffer[0]*0.072169; // Y=0.212671*R + 0.715160*G + 0.072169*B,???????;
	   *gray=0xff-(unsigned char)Y;
				...
				if(i%2==0)  //偶数行 ，从头到尾扫描
				{
					  //Y=21;
						for(j=0;j<x_max;j++)
						{
								 ...
								 Read_BMP(&Y,i,j);
								 // SERIAL_PROTOCOL(j);SERIAL_PROTOCOL(": ");SERIAL_PROTOCOLLN((unsigned int)Y);
								 if(Y>MIN_GRAY_VLAUE)
								 {
										 destination[X_AXIS]=-((float)x_max/2-j)*PIC_FIXED+80;
										 destination[Y_AXIS]= ((float)y_max/2-i)*PIC_FIXED+80;
										 line_to_destination(LASER_PRINT_SPEED);
										 set_current_to_destination();
										 while (planner.blocks_queued());
										   laser_status = 1;
									     time = Y*20;
										 WRITE(HEATER_0_PIN, 1);
										 while(time--);//烧灼的时候,根据灰度大小延时
										 WRITE(HEATER_0_PIN, 0);
										 idle();
							  }
						}
...
void prepare_laser_print()
{
  ...
  if(status == 0)
  {
	...
	enqueue_and_echo_commands_P(PSTR("G1 Z45"));//运动到制定高度
