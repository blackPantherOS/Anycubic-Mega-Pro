...
void Laset_print_picture(void)
{
  ...
  if(Laser_printer_st.pic_vector==0)
	...
	for(i=0;i<y_max;i++)
	{
		...
		destination[Y_AXIS]= 170 + (i -y_max/2)*Laser_printer_st.pic_pixel_distance;
		else
		destination[Y_AXIS]= 170 - (y_max/2  -i)*Laser_printer_st.pic_pixel_distance;	
		SERIAL_PROTOCOL(Laser_printer_st.pic_pixel_distance); SERIAL_PROTOCOL("   ");SERIAL_PROTOCOLLN(destination[Y_AXIS]) ;//
							...
							if(laser_status == 0)return ;
								 ...
								 if(Y>MIN_GRAY_VLAUE)
								 {
								       if(Laser_printer_st.pic_x_mirror==1)
										 destination[X_AXIS]=110+x_max/2-j*PIC_FIXED;
									   else
									   	destination[X_AXIS]=110-x_max/2+j*PIC_FIXED;
										 line_to_destination(ftemp);
										 set_current_to_destination();
						...
						for(j=0;j<x_max;j++)
						{
						 if(laser_status == 0)return ;
							...
							if(Y>MIN_GRAY_VLAUE) 
							 {								 
								    if(Laser_printer_st.pic_x_mirror==1)
									 destination[X_AXIS]=110+x_max/2-(x_max-j)*PIC_FIXED;
									else
									destination[X_AXIS]=110-x_max/2-(x_max-j)*PIC_FIXED;
...
void laser_indicate()//激光指示
{
  ...
  if(laser_on_off==1)
		...
		if(laser_indicate_steps == 0)
	{
		...
		if(Laser_printer_st.pic_x_mirror==1)
			{
			  x_start=110 - (Laser_printer_st.pic_widht * PIC_FIXED)/2;//X起始坐标
			  x_end=110 + (Laser_printer_st.pic_widht * PIC_FIXED)/2;
			}
		else
			{
             x_end =110 - (Laser_printer_st.pic_widht * PIC_FIXED)/2;//X起始坐标
			 x_start =110 + (Laser_printer_st.pic_widht * PIC_FIXED)/2;
		    }

		if(Laser_printer_st.pic_y_mirror==1)
			{
			  y_start=170 - (Laser_printer_st.pic_hight * PIC_FIXED)/2;//X起始坐标
			  y_end  =170 + (Laser_printer_st.pic_hight * PIC_FIXED)/2;
			}
		else
			{
             y_end =170 - (Laser_printer_st.pic_hight* PIC_FIXED)/2;//X起始坐标
			 y_start =170 + (Laser_printer_st.pic_hight * PIC_FIXED)/2;
		    }
...
void laser_init()
{
	...
	Laser_printer_st.pic_pixel_distance = 0.1;//像素之间的距离（0.1-0.3）
