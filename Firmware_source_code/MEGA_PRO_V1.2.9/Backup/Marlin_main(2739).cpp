...
/*
FRESULT Read_BMP(unsigned char nr,unsigned  char *gray,unsigned int y,unsigned int x)
{
     ...
     res = f_lseek(&fsrc, printer_st.pic_ptr);

		 
	 f_read(&fsrc, buffer,4 , &br);
	 ...
	 *gray=0xff-(unsigned char)Y;
	 ...
	 else if(printer_st.pic_bit==16)
	 {
		 ...
		 // printer_st.pic_ptr =(printer_st.pic_hight-y-1)*printer_st.pic_widht*2+x*2+printer_st.pic_start; 
      res = f_lseek(&fsrc, printer_st.pic_ptr);
	 	  f_read(&fsrc, buffer,2 , &br);
