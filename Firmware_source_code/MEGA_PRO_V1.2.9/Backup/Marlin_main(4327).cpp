...
char laser_on_off = 0;//ï¿½ï¿½ï¿½ï¿½Ö¸Ê¾ï¿½ï¿½ï¿½ï¿½×´Ì¬ï¿½ï¿½
char laser_status = 0 ;
char Laser_print_pause = 0;

char en_cycle_print = 0;
char seekdataflag=0;
...
NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_widht);//Í¼Æ¬ï¿½Ä¿ï¿½ï¿?
TFT_SERIAL_SPACE();
NEW_SERIAL_PROTOCOLPGM("H");
NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_hight);//Í¼Æ¬ï¿½Ä¸ß¶ï¿½
TFT_SERIAL_SPACE();
TFT_SERIAL_ENTER(); 
...
{
	NEW_SERIAL_PROTOCOLPGM("A44V");
	TFT_SERIAL_SPACE();
	NEW_SERIAL_PROTOCOLPGM("A");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_vector);
	TFT_SERIAL_SPACE();
	NEW_SERIAL_PROTOCOLPGM("B");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_laser_time);
	TFT_SERIAL_SPACE();
	NEW_SERIAL_PROTOCOLPGM("C");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.laser_height); 
	TFT_SERIAL_SPACE(); 
	NEW_SERIAL_PROTOCOLPGM("D");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_pixel_distance); 
	TFT_SERIAL_SPACE(); 
	NEW_SERIAL_PROTOCOLPGM("E");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.x_offset); 
	TFT_SERIAL_SPACE(); 
	NEW_SERIAL_PROTOCOLPGM("F");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.y_offset); 
	TFT_SERIAL_SPACE(); 
	NEW_SERIAL_PROTOCOLPGM("G");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_x_mirror); 
	TFT_SERIAL_SPACE();
    NEW_SERIAL_PROTOCOLPGM("H");
	NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_y_mirror); 
	TFT_SERIAL_SPACE();
	TFT_SERIAL_ENTER();  
       ...
       case 0://A0 GET HOTEND TEMP 
                       ...
                       //   temp=((fanSpeeds[0]*100)/179+1);
                         ...
                         case 5:// A5 GET CURRENT COORDINATE 
                          ...
                          TFTStatusFlag=1;
                        //  card.getStatus();
                        if(laser_status==1)//ï¿½ï¿½ï¿½ï¿½Ç¼ï¿½ï¿½ï¿½ï¿½Ó¡ï¿½ï¿½ï¿½Ç¼ï¿½ï¿½ã·½ï¿½ï¿½ï¿½ï¿½Í¬ï¿½ï¿½
                           {
                           char cbuf[15];
                           	int temp =100 -  100*Laser_printer_st.pic_ptr/Laser_printer_st.pic_file_size;
	                        sprintf(cbuf,"%d",temp);
                            NEW_SERIAL_PROTOCOL(cbuf);
						   }
						else
                           card.TFTgetStatus();
                          }
                          ...
                          NEW_SERIAL_PROTOCOLPGM("A7V ");
                           if(starttime != 0 && card.sdprinting) //print time
                          {
                ...
                //              else{
		                                    ...
		                                    TFT_SERIAL_ENTER();    
                           	               if(laser_status == 1)//¹Ø±Õ¼¤¹â
							              {    
							               WRITE(HEATER_0_PIN, 0);//ÔÝÍ£Ê±¹Ø±Õ¼¤¹â
                                           Laser_print_pause = 1;
		                                   NEW_SERIAL_PROTOCOLPGM("J18");//j05 pausing
		                                   TFT_SERIAL_ENTER();  
                           	              }
                         ...
                         if((!planner.movesplanned())&&(!TFTresumingflag))
                          {
                            ...
                            if(starpos!=NULL)
							 ...
							 file_type = 0;
							if(strstr(TFTstrchr_pointer,".bmp"))//Èç¹ûÊÇbmpÍ¼Æ¬ÎÄ¼þ
							{
							  file_type = 1;
							}
							  ...
							  if(file_type == 1)//Èç¹ûÍ¼Æ¬ÎÄ¼þµÄ»°£¬Ò»¶¨Òª°ÑÍ¼Æ¬ÐÅÏ¢·¢ËÍ¹ýÈ¥
							  send_pic_param();
							

                          }
	                else if(planner.movesplanned()&&en_continue==1)//Èç¹ûÔÚÔË¶¯»¹Ã»Íê³É£¬¾ÍÊÕµ½ÁËÐÅÏ¢£¬²¢ÇÒÔÊÐíÁ¬Ðø´òÓ¡
	                	{
	                	   SERIAL_PROTOCOLLN("*********open file cmd*******");
                              en_cycle_print = 1;
							  starpos = (strchr(TFTstrchr_pointer + 4,'*'));
							   SERIAL_PROTOCOLLN(TFTstrchr_pointer);
							  if(starpos!=NULL)
							  *(starpos-1)='\0';
							  next_file[0]  =  0;
							  strcat( next_file , TFTstrchr_pointer + 4);  //¿½±´ÎÄ¼þ
							  SERIAL_PROTOCOLLN(next_file);
							  SERIAL_PROTOCOLLN("****************");
					    }
                          break;
                         ...
                         // if((!USBConnectFlag)&&(!card.sdprinting))
                           laser_on_off = 0;
                         if((!planner.movesplanned())&&(!TFTresumingflag) || en_continue==1||file_type == 1)
                          {
                            card.startFileprint();
                            starttime=millis();
                   ...
                   if((!planner.movesplanned())&&(!TFTresumingflag))
                          {
                            ...
                            if(!TFTcode_seen('B')) enqueue_and_echo_commands_P(PSTR("G91"));   //Bï¿½ï¿½Ê¾ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ê£¬Ã»Bï¿½ï¿½Ê¾ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿?                                         
                            if(TFTcode_seen('X'))
                            {
                 ...
                 //        TFT_SERIAL_ENTER();
                       ...
                       if((!planner.movesplanned())&&(!TFTresumingflag))             
                         ...
                         break;
						   ...
						   case 32:// a32  stop cycle print
                         {
                             SERIAL_PROTOCOLPGM("stop print next mode ");
						     en_cycle_print = 0;
                         }
                         break;
                         ...
                         break;
						  ...
						  case 34:// Continuous printing Ê¹ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ó¡,Òªï¿½ï¿½Ò»Ð©×¼ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Í·ï¿½Â½ï¿½
                         {
                            en_continue = 1 ;}
                         
						  break;
						  case 35:// Continuous printing Ê§ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ó¡
                         {
                            en_continue = 0 ;}
                         
                         break;
						  
                         case 36://A36 Î»Í¼ ï¿½ï¿½ï¿½ï¿½Ê¸ï¿½ï¿½Í¼
						  if(TFTcode_seen('S'))
                         {
                          int coorvalue;
                          coorvalue=TFTcode_value_int();
						  if(coorvalue!=0)
						  Laser_printer_st.pic_vector = 1; //ï¿½ï¿½Ê¸ï¿½ï¿½Í¼
						  else
						  Laser_printer_st.pic_vector = 0;//ï¿½ï¿½Î»Í¼
						  	
                         }
						  case 37:
						  if(TFTcode_seen('S'))//Xï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
                         {
                          int coorvalue;
                          coorvalue=TFTcode_value_int();
						  if(coorvalue == 0)
						  ...
						  Laser_printer_st.pic_x_mirror = 1; //xï¿½ï¿½ï¿½ï¿½			  
                         }
                         	
						  break;
						  case 38:
                         if(TFTcode_seen('S'))//A38 ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ê±ï¿½ï¿½
                         {
                          int coorvalue;
                          coorvalue=TFTcode_value_int();
						 ...
						 case 39:
                         if(TFTcode_seen('S'))//A39
                         {
                          float coorvalue;
                          coorvalue=TFTcode_value();
						  Laser_printer_st.laser_height = coorvalue;
						  SERIAL_PROTOCOL("laser_height = ") ;
						  SERIAL_PROTOCOLLN(Laser_printer_st.laser_height) ;
						  
						 } 
 
						 break;
						  case 40:
                         if(TFTcode_seen('S'))//A40 
                         {
                          float coorvalue;
                          coorvalue=TFTcode_value();
                         ...
                         if(TFTcode_seen('S'))//y
                         {
						  ...
						  break;
						 case 51:// A51ï¿½Ô¶ï¿½ï¿½ï¿½Æ½
						  {
						     if(TFTcode_seen('H'))//ï¿½ï¿½Ò»ï¿½ï¿½
                            {
							 ...
							 else if(TFTcode_seen('I'))//ï¿½Ú¶ï¿½ï¿½ï¿½
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X190 Y30 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F300")); 
						    }
							  else if(TFTcode_seen('J'))//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X190 Y190 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F300"));                            
						    }
							   else if(TFTcode_seen('K'))//ï¿½ï¿½ï¿½Ä²ï¿½
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X30 Y190 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F300"));                            
						    }
							 else if(TFTcode_seen('L'))//ï¿½ï¿½ï¿½Ä²ï¿½
                            {
							 enqueue_and_echo_commands_P(PSTR("G1 X100 Y100  Z50 F5000"));  //ï¿½Øµï¿½ï¿½ï¿½ï¿½ï¿½Î»ï¿½ï¿½                           
						    }
						  }
						  break;
       default: break;
...
if(++time<2000)return;
	time = 0;
  if(!en_adjust_level)return;

   
  if(thermalManager.degHotend(0)>300&&thermalManager.degTargetHotend(0)<100)
  	{
...
if((thermalManager.degHotend(0)<5||thermalManager.degHotend(0)>285)&&en_adjust_level==0)   Scancount++;
...
static char temp=0;
 ...
 { 
   ...
   if(temp==2){enqueue_and_echo_commands_P(PSTR("G1 E-5"));} //»ØËõÒ»ÏÂ
   if(temp==3){enqueue_and_echo_commands_P(PSTR("G1 Z+20 E-5")); pauseCMDsendflag=false;temp=0;} //»ØËõÒ»ÏÂ
 }
}
/*
...
//ï¿½ï¿½ï¿½Ù´ï¿½Ó¡ï¿½ï¿½Ê±ï¿½ï¿½zï¿½ï¿½ï¿½Ì§ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ó³Ù£ï¿½ï¿½ï¿½Ê±ï¿½ï¿½Òªï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½È»Ø³é£¬È»ï¿½ï¿½ï¿½ï¿½ï¿½Í½ï¿½È¥ï¿½ï¿½ï¿½È¹ï¿½ï¿½ï¿½ï¿½ï¿½ã¡?
  if (IsRunning()) {
...
void Read_BMP(unsigned  char *gray,unsigned int y,unsigned int x)
{
	 ...
	 {
		 Laser_printer_st.pic_ptr =(Laser_printer_st.pic_hight-y-1)*Laser_printer_st.pic_realy_widht+x*4+Laser_printer_st.pic_start; 
         card.setIndex( Laser_printer_st.pic_ptr);
		 
	 card.read_bytes(buffer,4);
	 ...
	 *gray=0xff-(unsigned char)Y;
	 ...
	 else if(Laser_printer_st.pic_bit==24)
	 {
		...
		Laser_printer_st.pic_ptr =(unsigned long)(Laser_printer_st.pic_hight-y-1)*Laser_printer_st.pic_realy_widht+x*3+Laser_printer_st.pic_start;  
		card.setIndex( Laser_printer_st.pic_ptr);
		card.read_bytes(buffer,3);
	 ...
	 else if(Laser_printer_st.pic_bit==16)
	 {
		 ...
		 Laser_printer_st.pic_ptr =(Laser_printer_st.pic_hight-y-1)*Laser_printer_st.pic_realy_widht+x*2+Laser_printer_st.pic_start; 
	      card.setIndex( Laser_printer_st.pic_ptr);
	 	  card.read_bytes(buffer,2);
...
void Laset_print_picture(void)
{
  ...
  int x_max=Laser_printer_st.pic_widht,y_max=Laser_printer_st.pic_hight;
  ...
  if(Laser_printer_st.pic_vector==0)
	...
	ftemp = 2*LASER_PRINT_SPEED;//Ê¸Á¿Í¼¼ÓËÙ


	if(Laser_printer_st.pic_x_mirror==1)
		{
		  x_start = MAX_X_SIZE/2  + LASER_X_OFFSET + (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;//XÆðÊ¼×ø±ê
		  x_end   = MAX_X_SIZE/2  + LASER_X_OFFSET - (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;
		}
	else
		{
		 x_end   = MAX_X_SIZE/2  + LASER_X_OFFSET + (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;//XÆðÊ¼×ø±ê
		 x_start = MAX_X_SIZE/2  + LASER_X_OFFSET - (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;
		}
	
	if(Laser_printer_st.pic_y_mirror==1)
		{
		  y_start=MAX_Y_SIZE/2  + LASER_Y_OFFSET + (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;//XÆðÊ¼×ø±ê
		  y_end  =MAX_Y_SIZE/2  + LASER_Y_OFFSET - (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;
		}
	else
		{
		 y_end =MAX_Y_SIZE/2  + LASER_Y_OFFSET + (Laser_printer_st.pic_hight* Laser_printer_st.pic_pixel_distance)/2;//XÆðÊ¼×ø±ê
		 y_start =MAX_Y_SIZE/2  + LASER_Y_OFFSET - (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;
		}
	...
	for(i=0;i<y_max;i++)
	{
		...
		destination[Y_AXIS]= y_start + i*Laser_printer_st.pic_pixel_distance;
		else
		destination[Y_AXIS]=y_start  -  i*Laser_printer_st.pic_pixel_distance;	


		            if(i%2==0)	//Å¼ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½Í·ï¿½ï¿½Î²É¨ï¿½ï¿½
						{
								for(j=0;j<x_max;j++)
								{
										 Read_BMP(&Y,i,j);
										 ...
										 {
											   if(Laser_printer_st.pic_x_mirror==1)
												 destination[X_AXIS]=x_start-j*Laser_printer_st.pic_pixel_distance;
											   else
												 destination[X_AXIS]=x_start+j*Laser_printer_st.pic_pixel_distance;
				 ...
				 WRITE(HEATER_0_PIN, 0); 
					  ...
					  //Y=21;
						for(j=0;j<x_max;j++)
						{
							if(laser_status == 0)return ;
								 Read_BMP(&Y,i,j);
								 if(Y>MIN_GRAY_VLAUE && laser_on ==false )
								 {
								       if(Laser_printer_st.pic_x_mirror==1)
										 destination[X_AXIS]=x_start-j*Laser_printer_st.pic_pixel_distance;
									   else
									   	destination[X_AXIS]=x_start+j*Laser_printer_st.pic_pixel_distance;
									    line_to_destination(5*LASER_PRINT_SPEED);
										 set_current_to_destination();
										 while (planner.blocks_queued());
										 if(Laser_printer_st.pic_vector==0)
										{
...
void prepare_laser_print()
{
  ...
  {
	cvalue[0]= 0;
	while (planner.blocks_queued());//µÈ´ýÔË¶¯Í£Ö¹
    enqueue_and_echo_commands_P(PSTR("G28"));//ÏÈ¹éÁãÔÙÔË¶¯µ½ÖÆ¶¨¸ß¶È
    sprintf_P(cvalue,PSTR("G1 Z%i F500"),(int)Laser_printer_st.laser_height);
	
SERIAL_PROTOCOLLN(cvalue);
	enqueue_and_echo_command_now(cvalue);//ÔË¶¯µ½ÖÆ¶¨¸ß¶È
	status =1;
    times = 120000;
  }
  else if(status==1)
  {
   if(planner.blocks_queued())return;//Èç¹û»¹ÔÚÔË¶¯,Ôò·µ»Ø
   status =2;
  }
    else if(status==2)
  {
   Laset_print_picture( );//¿ªÊ¼¼¤¹â´òÓ¡
   status=0;
   card.printingHasFinished();
   card.checkautostart(true);
   en_continue = 0;
...
void process_next_command() {
  ...
  switch (command_code) {
    ...
    case 'M': switch (codenum) {
      ...
      // if(en_continue)//Á¬Ðø´òÓ¡Ä£Ê½ÏÂ£¬¸ÄÓÃÕâÖÖÉýÎÂ·½Ê½
	  // 	   gcode_M104();
	   //else
        gcode_M109();
        break;
...
void laser_indicate()//ï¿½ï¿½ï¿½ï¿½Ö¸Ê¾
{
 static unsigned char laser_indicate_steps = 0 ;
  static unsigned long times = 0;
  if(times>100)
  	{
  	 times -- ;
  ...
  if(laser_on_off==1)
    ...
    {
       laser_indicate_steps = 0 ;
	 	return;
	}
  	
  if(file_type == 0)return;

		if(laser_indicate_steps == 0)
	{
		
	    //while (planner.blocks_queued());//µÈ´ýÔË¶¯Í£Ö¹	
	    WRITE(HEATER_0_PIN, 0);
		enqueue_and_echo_commands_P(PSTR("G28 X0 Y0"));//ÏÈ¹éÁãÔÙÔË¶¯µ½ÖÆ¶¨¸ß¶È
	    ...
	    /*	 
	     cvalue[0]= 0;
		 sprintf_P(cvalue,PSTR("G1 X%i Y%i F1000"),(int)x_start,(int)y_start);
		 SERIAL_PROTOCOLLN(cvalue);
		 enqueue_and_echo_command_now(cvalue);//ÔË¶¯µ½ÖÆ¶¨¸ß¶È
		 */
		 times = 100000;
		 laser_indicate_steps = 1;

		}
		else if(laser_indicate_steps == 1)
		{
		if(planner.blocks_queued())return;
		destination[X_AXIS]= x_end ;
		destination[Z_AXIS]= current_position[Z_AXIS] ;//·ÀÖ¹ZÏÂ½µ
		SERIAL_PROTOCOLLNPAIR("1 x=", x_end);
		 line_to_destination(60);
		 set_current_to_destination();
		laser_indicate_steps = 2;

		}
		else if(laser_indicate_steps == 2)
		{
			if(planner.blocks_queued())return;
			destination[Y_AXIS]= y_end ;
			
		 SERIAL_PROTOCOLLNPAIR("2 y=", y_end);
			 line_to_destination(60);
			 set_current_to_destination();
			laser_indicate_steps = 3;
		}
			else if(laser_indicate_steps == 3)
		{
			if(planner.blocks_queued())return;
			destination[X_AXIS]= x_start ;
			 SERIAL_PROTOCOLLNPAIR("3 x=", x_start);
			 line_to_destination(60);
			 set_current_to_destination();
			laser_indicate_steps = 4;		
		}
			else if(laser_indicate_steps == 4)
		{
			if(planner.blocks_queued())return;
			destination[Y_AXIS]= y_start ;
		 SERIAL_PROTOCOLLNPAIR("4 y=", y_start);
			 line_to_destination(60);
			 set_current_to_destination();
			laser_indicate_steps = 1;
		}
...
void print_next_mode()
{
	char *starpos = NULL;
	static  millis_t time_save  = 0;
	if(millis()<time_save)return;
	time_save = millis()+3000;//5ÃëÖ´ÐÐÒ»´Î
		

 if(!en_cycle_print )return ;
 if(planner.movesplanned()){

	 
	time_save += 5000;//Ôö¼Ó3,×÷ÓÃÊÇÎªÁË°ÑÏÂÒ»¸ö¶¯×÷Ö´ÐÐ½áÊø
	return;//Èç¹ûÔË¶¯Ã»½áÊøÍê³É£¬Ôò·µ»Ø£¬Èç¹ûÔË¶¯½áÊøÁË£¬¾Í¿ÉÒÔ×¼±¸ÏÂÒ»¸ö´òÓ¡ÁË¡£
 }
// if(thermalManager.degBed()>40)
 	//{
 	//SERIAL_PROTOCOLLN("Wait for the temperature to drop below 40 degrees.......");
 	//return;//Èç¹ûÈÈ´²ÎÂ¶È»¹ºÜ¸ß£¬ÄÇÃ´¾ÍµÈ´ý
 	//}
    en_cycle_print = 0;

    SERIAL_PROTOCOLLN("*0");
    do_blocking_move_to_xy(25, 200);
	SERIAL_PROTOCOLLN("*1");
	do_blocking_move_to_z(2);
	SERIAL_PROTOCOLLN("*2");
	do_blocking_move_to_xy(25, 10,2000/60);

	SERIAL_PROTOCOLLN("*4");

    do_blocking_move_to_xy(110, 200);
	SERIAL_PROTOCOLLN("*5");
	do_blocking_move_to_xy(110, 10,2000/60);
	SERIAL_PROTOCOLLN("*6");

    do_blocking_move_to_xy(180, 200);
	SERIAL_PROTOCOLLN("*7");
	do_blocking_move_to_xy(180, 10,2000/60);
	
	 //enqueue_and_echo_commands_P(PSTR("G1 Y200")); 
	 //enqueue_and_echo_commands_P(PSTR("G1 Z2")); 
	// SERIAL_PROTOCOLLN("*1");
	 //enqueue_and_echo_commands_P(PSTR("G1 Y10 F1000")); 
	// SERIAL_PROTOCOLLN("*2");
 
  card.openFile(next_file,true);
  sdcardstartprintingflag=1;
  TFT_SERIAL_ENTER();
...
void laser_init()
{
	...
	Laser_printer_st.pic_pixel_distance = PIC_FIXED;//ÏñËØÖ®¼äµÄ¾àÀë£¨0.1-0.3£©
	Laser_printer_st.laser_height = 50;//¼¤¹â¸ß¶È
	Laser_printer_st.x_offset = 0;//X·½ÏòµÄÆ«ÒÆ
	Laser_printer_st.x_offset = 0;//Y·½ÏòµÄÆ«ÒÆ
	
	Laser_printer_st.pic_vector = 0;//ÊÇ·ñÊÇÊ¸Á¿Í¼£¬1ÎªÊ¸Á¿Í¼¡£0ÎªÎ»Í¼
	Laser_printer_st.pic_x_mirror = 1; //X·½Ïò¾µÏñ,1Îª·´Ïò
	Laser_printer_st.pic_y_mirror = 0; //Y·½Ïò¾µÏñ£¬1Îª·´Ïò
	Laser_printer_st.pic_laser_time = 50;//¼¤¹âÕÕÉäÊ±¼ä£¨1-100£©
...
//  if((AssistLeveTestflag==1)&&(commands_in_queue < BUFSIZE)){ AssistLevelTest();}
  ...
  if(file_type==1&&card.sdprinting==true)
  	{
      prepare_laser_print();//ï¿½ï¿½ï¿½ï¿½Òªï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½È¹ï¿½ï¿½ã£¬ï¿½ï¿½ï¿½Ë¶ï¿½ï¿½ï¿½Ô¤ï¿½ï¿½ï¿½ß¶È£ï¿½È»ï¿½ï¿½Å¿ï¿½Ê¼ï¿½ï¿½ï¿½ï¿½ï¿½Ó?
    }
}
