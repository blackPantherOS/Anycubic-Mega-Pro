...
NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_widht);//图片的宽度
TFT_SERIAL_SPACE();
NEW_SERIAL_PROTOCOLPGM("H");
NEW_SERIAL_PROTOCOL(Laser_printer_st.pic_hight);//图片的高度
TFT_SERIAL_SPACE();
TFT_SERIAL_ENTER(); 
       ...
       case 0://A0 GET HOTEND TEMP 
                       ...
                       //   temp=((fanSpeeds[0]*100)/179+1);
                         ...
                         case 5:// A5 GET CURRENT COORDINATE 
                          ...
                          TFTStatusFlag=1;
                        //  card.getStatus();
                        if(laser_status==1)//如果是激光打印，那计算方法不同了
                           {
                           char cbuf[15];
                ...
                //              else{
                    ...
                    //               }                       
                          ...
                          {    
                          	if(laser_status == 1)//关闭激光
							  {
							    enqueue_and_echo_commands_P(PSTR("G6 S0")); 
								laser_status = 0;
							  }
                         ...
                         if((!planner.movesplanned())&&(!TFTresumingflag))
                          {
                            ...
                            if(starpos!=NULL)
							 ...
							 file_type = 0;
							if(strstr(TFTstrchr_pointer,".bmp"))//如果是bmp图片文件
							{
							  file_type = 1;
							}
							  ...
							  if(file_type == 1)//如果图片文件的话，一定要把图片信息发送过去
							  send_pic_param();
							

                          }
	                else if(planner.movesplanned()&&en_continue==1)//如果在运动还没完成，就收到了信息，并且允许连续打印
	                	{
	                	   SERIAL_PROTOCOLLN("*********open file cmd*******");
							  ...
							  *(starpos-1)='\0';
							  next_file[0]  =  0;
							  strcat( next_file , TFTstrchr_pointer + 4);  //拷贝文件
                         ...
                         // if((!USBConnectFlag)&&(!card.sdprinting))
                           laser_on_off = 0;
						   SERIAL_PROTOCOLLN("*********start print cmd*******");
                   ...
                   if((!planner.movesplanned())&&(!TFTresumingflag))
                          {
                            ...
                            if(!TFTcode_seen('B')) enqueue_and_echo_commands_P(PSTR("G91"));   //B表示绝对坐标，没B表示相对坐标                                          
                            if(TFTcode_seen('X'))
                            {
                 ...
                 //        TFT_SERIAL_ENTER();
                       ...
                       if((!planner.movesplanned())&&(!TFTresumingflag))             
                         ...
                         break;
						  ...
						  case 34:// Continuous printing 使能连续打印,要做一些准备工作，喷头下降
                         {
                            en_continue = 1 ;}
                         
						  break;
						  case 35:// Continuous printing 失能连续打印
                         {
                            en_continue = 0 ;}
                         
                         break;
						  
                         case 36://A36 位图 或者矢量图
						  if(TFTcode_seen('S'))
                         {
                          int coorvalue;
                          coorvalue=TFTcode_value_int();
						  if(coorvalue!=0)
						  Laser_printer_st.pic_vector = 1; //是矢量图
						  else
						  Laser_printer_st.pic_vector = 0;//是位图
						  	
                         }
						  case 37:
						  if(TFTcode_seen('S'))//X方向镜像
                         {
                          int coorvalue;
                          coorvalue=TFTcode_value_int();
						  if(coorvalue = 0)
						  ...
						  Laser_printer_st.pic_x_mirror = 1; //x镜像			  
                         }
                         	
						  break;
						  case 38:
                         if(TFTcode_seen('S'))//A38 激光照射时间
                         {
                          int coorvalue;
                          coorvalue=TFTcode_value_int();
                         ...
                         if(TFTcode_seen('S'))//y方向镜像
                         {
						  ...
						  Laser_printer_st.pic_y_mirror = 1; //y镜像
						  				  
                         }
						  break;
						  case 44:
                          send_laser_param();//发送激光打印数据
						  break;
						 ...
						 case 51:// A51自动调平
						  {
						     if(TFTcode_seen('H'))//第一步
                            {
							 ...
							 else if(TFTcode_seen('I'))//第二步
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X190 Y30 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F300")); 
						    }
							  else if(TFTcode_seen('J'))//第三步
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X190 Y190 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F300"));                            
						    }
							   else if(TFTcode_seen('K'))//第四步
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X30 Y190 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F300"));                            
						    }
							 else if(TFTcode_seen('L'))//第四步
                            {
							 enqueue_and_echo_commands_P(PSTR("G1 X100 Y100  Z50 F5000"));  //回到中心位置                           
						    }
						  }
						  break;
       default: break;
...
void gcode_get_destination() {
  ...
  LOOP_XYZE(i) {
    if (code_seen(axis_codes[i]))
	 ...
	 if (code_seen('Z')&&is_pumpback==0)//如果发现是Z轴运动，那么一定要先Z先回抽
	{
	   destination[E_AXIS] = current_position[E_AXIS]-2;//回抽2mm  
	   is_pumpback=1;
	}
...
//高速打印的时候，z轴的抬升会带来延迟，这时候，要把物料先回抽，然后再送进去，度过这个点。
  if (IsRunning()) {
   //if(is_pumpback != 2) 
        ...
        if((ResumingFlag==1)&&FlagResumFromOutage)
    {
	  ...
	  else if(is_pumpback==1)
	  {
	    is_pumpback = 2 ;
        prepare_move_to_destination();
	  }
	  else if(is_pumpback == 2)
	  {
...
void Laset_print_picture(void)
{
  ...
  if(Laser_printer_st.pic_vector==0)
	...
	ftemp = 2*LASER_PRINT_SPEED;//矢量图加速


	if(Laser_printer_st.pic_x_mirror==1)
		{
		  x_start=110 + (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;//X起始坐标
		  x_end=110 - (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;
		}
	else
		{
		 x_end =110 + (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;//X起始坐标
		 x_start =110 - (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;
		}
	
	if(Laser_printer_st.pic_y_mirror==1)
		{
		  y_start=170 + (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;//X起始坐标
		  y_end  =170 - (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;
		}
	else
		{
		 y_end =170 + (Laser_printer_st.pic_hight* Laser_printer_st.pic_pixel_distance)/2;//X起始坐标
		 y_start =170 - (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;
		}
...
void prepare_laser_print()
{
  ...
  {
	cvalue[0]= 0;
	while (planner.blocks_queued());//等待运动停止
    enqueue_and_echo_commands_P(PSTR("G28"));//先归零再运动到制定高度
    sprintf_P(cvalue,PSTR("G1 Z%i F500"),(int)Laser_printer_st.laser_height);
	SERIAL_PROTOCOLLN(cvalue);
	enqueue_and_echo_command_now(cvalue);//运动到制定高度
	status =1;
  }
  else if(status==1)
  {
   if(planner.blocks_queued())return;//如果还在运动,则返回
   status =2;
  }
    else if(status==2)
  {
   Laset_print_picture( );//开始激光打印
   status=0;
   card.printingHasFinished();
   card.checkautostart(true);
   en_continue = 0;
  ...
  switch (command_code) {
    ...
    case 'M': switch (codenum) {
      ...
      case 109: // M109: Wait for hotend temperature to reach target
       if(en_continue)//连续打印模式下，改用这种升温方式
	   	   gcode_M104();
	   else
        gcode_M109();
        break;
 ...
 static unsigned char laser_indicate_steps = 0 ;
  ...
  if(file_type == 0)return;
		...
		if(laser_indicate_steps == 0)
	{
	    ...
	    WRITE(HEATER_0_PIN, 0);
		enqueue_and_echo_commands_P(PSTR("G28 X0 Y0"));//先归零再运动到制定高度
		...
		destination[Z_AXIS]= current_position[Z_AXIS] ;//防止Z下降
...
void print_next_mode()
{
 ...
 if(!planner.movesplanned())return;//如果运动没结束完成，则返回，如果运动结束了，就可以准备下一个打印了。
...
void laser_init()
{
	...
	Laser_printer_st.pic_pixel_distance = PIC_FIXED;//像素之间的距离（0.1-0.3）
	Laser_printer_st.laser_height = 50;//激光高度
	Laser_printer_st.x_offset = 0;//X方向的偏移
	Laser_printer_st.x_offset = 0;//Y方向的偏移
	
	Laser_printer_st.pic_vector = 0;//是否是矢量图，1为矢量图。0为位图
	Laser_printer_st.pic_x_mirror = 1; //X方向镜像,1为反向
	Laser_printer_st.pic_y_mirror = 0; //Y方向镜像，1为反向
	Laser_printer_st.pic_laser_time = 15;//激光照射时间（10-30）
...
//  if((AssistLeveTestflag==1)&&(commands_in_queue < BUFSIZE)){ AssistLevelTest();}
  ...
  if(file_type==1&&card.sdprinting==true)
  	{
      prepare_laser_print();//这里要做的事情就是先归零，再运动到预定高度，然后才开始激光打印
    }
}
