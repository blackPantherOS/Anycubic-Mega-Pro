    ...
    //     NEW_SERIAL_ERRORLN(gcode_LastN);
       ...
       case 0://A0 GET HOTEND TEMP 
                       ...
                       //   temp=((fanSpeeds[0]*100)/179+1);
                        ...
                        if(laser_status==1)//如果是激光打印，那计算方法不同了
                           {
                           char cbuf[5];
                           	int temp = 100*Laser_printer_st.pic_ptr/Laser_printer_st.pic_file_size;
	                        sprintf(cbuf,"%d",temp);
      ...
      if((!planner.movesplanned())&&(!TFTresumingflag))
	                ...
	                else if(planner.movesplanned()&&en_continue==1)//如果在运动还没完成，就收到了信息，并且允许连续打印
	                	{
                         ...
                         // if((!USBConnectFlag)&&(!card.sdprinting))
                         laser_on_off = 0;
                         if((!planner.movesplanned())&&(!TFTresumingflag) || en_continue==1)
                          {
                            card.startFileprint();
                            starttime=millis();
...
/*
void Laser_indicate()//激光范围指示
{

   unsigned int x_max=printer_st.pic_widht,y_max=printer_st.pic_hight;
   float laser_pos_start[2];
   float laser_pos_end[2];

   if(printer_st.pic_x_mirror==1)
   	{
	   destination[X_AXIS]=155-j*PIC_FIXED;
	   laser_pos_start[0] = ;
	   laser_pos_end[0];
   	...
   	{
	   laser_pos_start[0] = 110-(x_max*PIC_FIXED/2);
	   laser_pos_end[0] = 110 + (x_max*PIC_FIXED/2);
   	}

      if(printer_st.pic_y_mirror==1)
   	{
	  destination[Y_AXIS]= 123 - i*PIC_FIXED;
	  	   laser_pos_start[1];
	   laser_pos_end[1];
   	...
   	{
   	  destination[Y_AXIS]= 123 + i*PIC_FIXED;
	  	   laser_pos_start[1] = 110-(x_max*PIC_FIXED/2);
	   laser_pos_end[1];
   	}
...
void prepare_laser_print()
{
  ...
  if(status == 0)
  {
    ...
    sprintf_P(cvalue,PSTR("G1 Z%f F500"),Laser_printer_st.laser_height);
	enqueue_and_echo_commands_P(cvalue);//运动到制定高度
	status =1;
  }
  else if(status==1)
  {
  ...
  if(file_type == 0)return;
		...
		if(laser_indicate_steps == 0)
	{
		...
		if(Laser_printer_st.pic_y_mirror==1)
		 ...
		 sprintf(cvalue,PSTR("G1 X%f Y%f F3000"),x_start,y_start); 
		 SERIAL_PROTOCOLLN(cvalue);
		 enqueue_and_echo_commands_P(cvalue);
