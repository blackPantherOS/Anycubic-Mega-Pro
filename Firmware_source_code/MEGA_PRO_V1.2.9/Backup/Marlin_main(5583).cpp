      ...
      if((!planner.movesplanned())&&(!TFTresumingflag))
	                ...
	                else if(planner.movesplanned()&&en_continue==1)//如果在运动还没完成，就收到了信息，并且允许连续打印
	                	{
                          en_cycle_print = 1;
						   ...
						   //while(planner.movesplanned()) ;//等待结束
                         ...
                         // if((!USBConnectFlag)&&(!card.sdprinting))
      if((!planner.movesplanned())&&(!TFTresumingflag) || en_cycle_print == 1)
                          {
                            en_cycle_print == 0 ;
                            card.startFileprint();
                            starttime=millis();
...
void print_next_mode()
{
 ...
 do_blocking_move_to_z(100,20);//向上运动
 do_blocking_move_to_xy(100, 200,100);//再移动到Y轴的最后后面
 do_blocking_move_to_z(10,20);//再下来
 do_blocking_move_to_xy(100,0,100);//撞掉模型
