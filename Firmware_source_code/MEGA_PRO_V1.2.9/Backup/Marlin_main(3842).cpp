       ...
       case 0://A0 GET HOTEND TEMP 
                       ...
                       //   temp=((fanSpeeds[0]*100)/179+1);
                        ...
                        if(laser_status==1)//如果是激光打印，那计算方法不同了
                           {
                           char cbuf[5];
                           	int temp = 100*printer_st.pic_ptr/printer_st.pic_file_size;
	                        sprintf(cbuf,"%d",temp);
...
void Read_BMP(unsigned  char *gray,unsigned int y,unsigned int x)
{
	 ...
	 if(printer_st.pic_bit==32)
	 {
		 printer_st.pic_ptr =(printer_st.pic_hight-y-1)*printer_st.pic_realy_widht+x*4+printer_st.pic_start; 
         card.setIndex( printer_st.pic_ptr);
	 ...
	 *gray=0xff-(unsigned char)Y;
	 ...
	 else if(printer_st.pic_bit==24)
	 {

		printer_st.pic_ptr =(unsigned long)(printer_st.pic_hight-y-1)*printer_st.pic_realy_widht+x*3+printer_st.pic_start;  
		card.setIndex( printer_st.pic_ptr);
	 ...
	 else if(printer_st.pic_bit==16)
	 {
		 

		 printer_st.pic_ptr =(printer_st.pic_hight-y-1)*printer_st.pic_realy_widht+x*2+printer_st.pic_start; 
	      card.setIndex( printer_st.pic_ptr);
...
void Laset_print_picture(void)
{
  ...
  laser_status = 1;
  if(printer_st.pic_vector==0)
  	ftemp = LASER_PRINT_SPEED;
  	else
	ftemp = 3*LASER_PRINT_SPEED;//矢量图加速
		
	//static float destination_temp[4];
	//L_LED_SET(1);
	for(i=0;i<y_max;i++)
	{
	    
		
		destination[Y_AXIS]= i*PIC_FIXED+123;
					  ...
					  //Y=21;
						for(j=0;j<x_max;j++)
						{
							if(laser_status == 0)return ;
								 ...
								 if(Y>MIN_GRAY_VLAUE)
								 {
										 destination[X_AXIS]=155-j*PIC_FIXED;//-((float)x_max/2-j)*PIC_FIXED+80;
										 //((float)y_max/2-i)*PIC_FIXED+80;
										 line_to_destination(ftemp);
										 set_current_to_destination();
						 ...
						 if(laser_status == 0)return ;
							...
							if(Y>MIN_GRAY_VLAUE) 
							 {								 
									 ...
									 destination[X_AXIS]=155-(x_max-j)*PIC_FIXED;//((float)x_max/2-j)*PIC_FIXED+80;
									 line_to_destination(ftemp);
									 set_current_to_destination();
									 while (planner.blocks_queued());
									 WRITE(HEATER_0_PIN, 1);
