      ...
      if((!planner.movesplanned())&&(!TFTresumingflag))
                     ...
                     //    }
                         ...
                         break;
						  ...
						  case 34:// Continuous printing 使能连续打印
                         {
                            en_continue = 1 ;
                         }
                         break;
...
// 如果最低速度F33
// 那就相当于一分钟转1圈，3200*1/60 = 53HZ

// 如果F1000
// 一分钟转圈30圈           	  3200*30/60 = 1616HZ

// 所以只有F6000              (6000/60/33)*3200 = 9696HZ  ; 


//要设置一个定时器，以1微妙为单位，然后捕获这个脉冲宽度，由脉宽可以反推出速度大小，这个目标速度就是精确的目标速度了。
			 
	  if(flag_time_update )//如果超时速度重置为零
	   {
		 flag_time_update=0;
		 cycle_time = 0;
		 cmd_speed = 0;
	...
	if(flag_capt)//输入捕获标志置位
	{
	  flagcapt = 0;
	  cycle_time  = (65536+get_capture()- cycle_time)%65536;
	  if(cycle_time==0)
	  cmd_speed  = 0;
	  else
	  cmd_speed = 512/cycle_time ;
