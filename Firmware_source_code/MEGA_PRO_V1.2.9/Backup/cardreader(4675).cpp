...
extern PRINTER_STRUCT   Laser_printer_st;
        ...
        if((MyFileNrCnt-filenumber)<4)
        {
          if(fileoutputcnt<(MyFileNrCnt-filenumber))
          {
           ...
           NEW_SERIAL_PROTOCOL(prepend);
			...
			while(filename[i_temp]!='.')
		    {
			  p_temp = (unsigned char *)(&filename[i_temp]);
             if(*p_temp>128)
              {
              NEW_SERIAL_PROTOCOLLN(filename);
              goto here1 ;
...
void CardReader::startFileprint() {
  ...
  if(TFTresumingflag)
    {   
      enqueue_and_echo_commands_P(PSTR("G1 Z-20 E10"));
...
//      NEW_SERIAL_PROTOCOLLNPGM(MSG_SD_FILE_SELECTED);
      ...
      if(file_type==1)//如果是bmp文件，就要把很多信息读出来
		...
		Laser_printer_st.pic_widht=(unsigned int)st_bmp.biWidth[0]+(unsigned int)st_bmp.biWidth[1]*256;
		Laser_printer_st.pic_widht_odd=Laser_printer_st.pic_widht%2;
		//Laser_printer_st.pic_hight= *ppp;
		Laser_printer_st.pic_hight=(unsigned int)st_bmp.biHeight[0]+(unsigned int)st_bmp.biHeight[1]*256;
		if(Laser_printer_st.pic_hight>30000)
		{Laser_printer_st.pic_hight = 65536-Laser_printer_st.pic_hight;Laser_printer_st.pic_dir = 1;
			}
		...
		Laser_printer_st.pic_realy_widht=(((Laser_printer_st.pic_widht*Laser_printer_st.pic_bit)+31)>>5)<<2;//计算每行的字节数
		setIndex(54);
		...
		SERIAL_PROTOCOL("pic_hight: "); SERIAL_PROTOCOLLN(Laser_printer_st.pic_hight);
		SERIAL_PROTOCOL("pic_bit:	"); SERIAL_PROTOCOLLN((int)Laser_printer_st.pic_bit);
		SERIAL_PROTOCOL("pic_ptr:	"); SERIAL_PROTOCOLLN(Laser_printer_st.pic_ptr);
		SERIAL_PROTOCOL("realy_widht:"); SERIAL_PROTOCOLLN(Laser_printer_st.pic_realy_widht);
