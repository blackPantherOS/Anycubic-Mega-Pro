...
{		/*图片打印部分参数**/
	...
	float pic_pixel_distance;//像素之间的距离（0.1-0.3）
	float laser_height;//激光高度
	float x_offset;//X方向的偏移
	float y_offset;//Y方向的偏移
	
	unsigned int  pic_realy_widht;
	unsigned int  pic_widht;
 	unsigned int  pic_hight;
	unsigned char pic_bit;
	unsigned char pic_widht_odd; //奇偶数
	unsigned char pic_hight_odd; //奇偶数
	
	unsigned char pic_print_status; //打印状态
	unsigned char pic_dir;//图片方向
	
	unsigned char pic_vector;//是否是矢量图，1为矢量图。0为位图
	unsigned char pic_x_mirror; //X方向镜像,1为反向
	unsigned char pic_y_mirror; //Y方向镜像，1为反向
	unsigned char pic_laser_time;//激光照射时间（10-30）
...
#define MIN_GRAY_VLAUE  20  //最小的灰度值 小于该值不点亮激光
