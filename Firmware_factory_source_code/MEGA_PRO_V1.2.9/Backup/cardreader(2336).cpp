...
extern char laser_status ;

extern PRINTER_STRUCT   printer_st;
extern BMP_HEAD st_bmp;
...
//          #endif
    ...
    else
    {
      ...
      if(!filenameIsDir)
      {
        if(p.name[8]!='G'&&p.name[8]!='B') continue;//
      ...
      if(ReadMyfileNrFlag)
        {
           /*char * p1 = p.name ;
            ...
            SERIAL_PROTOCOLLN((const char *)p1);
			
			SERIAL_PROTOCOL(prepend);
            SERIAL_PROTOCOLLN(filename);
            SERIAL_PROTOCOLLN(longFilename);*/
        if((strstr(filename,".gco")!=NULL)||(strstr(filename,".GCO")!=NULL)||(strstr(filename,".bmp")!=NULL)||(strstr(filename,".BMP")!=NULL))MyFileNrCnt++;
        }
      else 
      {
        // SERIAL_PROTOCOLLNPAIR("MyFileNrCnt = ", MyFileNrCnt);
	     //SERIAL_PROTOCOLLNPAIR("fileoutputcnt = ", fileoutputcnt);
	    // SERIAL_PROTOCOLLNPAIR("filenumber = ", filenumber);
...
//      NEW_SERIAL_PROTOCOLLNPGM(MSG_SD_FILE_SELECTED);
      ...
      if(file_type==1)//如果是bmp文件，就要把很多信息读出来
      	{
      	unsigned char * pp =NULL;
		//long * ppp =(long *)(&st_bmp.biHeight[0]);
		
		    printer_st.pic_dir = 0;
      		read_bytes(&st_bmp, sizeof(st_bmp));
			printer_st.pic_file_size=(unsigned long)st_bmp.bfSize[0]+(unsigned long)st_bmp.bfSize[1]*256+(unsigned long)st_bmp.bfSize[2]*256*256+(unsigned long)st_bmp.bfSize[3]*256*256*256;
			printer_st.pic_widht=(unsigned int)st_bmp.biWidth[0]+(unsigned int)st_bmp.biWidth[1]*256;
			printer_st.pic_widht_odd=printer_st.pic_widht%2;
			//printer_st.pic_hight= *ppp;
			printer_st.pic_hight=(unsigned int)st_bmp.biHeight[0]+(unsigned int)st_bmp.biHeight[1]*256;
			if(printer_st.pic_hight>30000)
			{printer_st.pic_hight = 65536-printer_st.pic_hight;printer_st.pic_dir = 1;
				}
			printer_st.pic_bit=st_bmp.biBitCount[0];
			printer_st.pic_start=54;
			printer_st.pic_ptr=printer_st.pic_widht*printer_st.pic_hight*printer_st.pic_bit/8+printer_st.pic_start;
			printer_st.pic_realy_widht=(((printer_st.pic_widht*printer_st.pic_bit)+31)>>5)<<2;//计算每行的字节数
			setIndex(54);
			...
			for(int i = 0;i<54 ;i++)
		    {
				SERIAL_PROTOCOLLN((int)(pp[i]));
			}
			SERIAL_PROTOCOLLN("****************************************");

	        SERIAL_PROTOCOL("file size:");  SERIAL_PROTOCOLLN(printer_st.pic_file_size);
			SERIAL_PROTOCOL("pic_widht: "); SERIAL_PROTOCOLLN(printer_st.pic_widht);
			SERIAL_PROTOCOL("pic_hight: "); SERIAL_PROTOCOLLN(printer_st.pic_hight);
			SERIAL_PROTOCOL("pic_bit:   "); SERIAL_PROTOCOLLN((int)printer_st.pic_bit);
			SERIAL_PROTOCOL("pic_ptr:   "); SERIAL_PROTOCOLLN(printer_st.pic_ptr);
			SERIAL_PROTOCOL("realy_widht:"); SERIAL_PROTOCOLLN(printer_st.pic_realy_widht);
			SERIAL_PROTOCOLLN("****************************************");
