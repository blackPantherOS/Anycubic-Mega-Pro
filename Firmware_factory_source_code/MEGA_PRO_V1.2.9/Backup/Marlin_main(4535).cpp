                 ...
                 //        TFT_SERIAL_ENTER();
                       ...
                       if((!planner.movesplanned())&&(!TFTresumingflag))             
                         ...
                         if(TFTcode_seen('S'))//抬升激光头到制定高度
                         {
                          ...
                          sprintf_P(value,PSTR("G1 Z%iF500"),int(coorvalue)); 
						 ...
						 case 51:// A51自动调平
						  {
						     if(TFTcode_seen('H'))//第一步
                            {
							 ...
							 enqueue_and_echo_commands_P(PSTR("G1 X10 Y10 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F500")); 
						    }
							 else if(TFTcode_seen('I'))//第二步
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X200 Y0 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F500")); 
						    }
							  else if(TFTcode_seen('J'))//第三步
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X200 Y200 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F500"));                            
						    }
							   else if(TFTcode_seen('K'))//第四步
                            {
                              enqueue_and_echo_commands_P(PSTR("G1 Z5 F500")); 
							 enqueue_and_echo_commands_P(PSTR("G1 X0 Y200 F5000")); 
							 enqueue_and_echo_commands_P(PSTR("G1 Z0 F500"));                            
						    }
						  }
						  break;
...
void Laset_print_picture(void)
{
  ...
  if(Laser_printer_st.pic_vector==0)
	...
	ftemp = 3*LASER_PRINT_SPEED;//矢量图加速
						...
						for(j=0;j<x_max;j++)
						{
							if(laser_status == 0)return ;
								 ...
								 if(Y>MIN_GRAY_VLAUE)
								 {
									   ...
									   //SERIAL_PROTOCOL(destination[X_AXIS]) ; SERIAL_PROTOCOL("	");SERIAL_PROTOCOLLN(destination[Y_AXIS]) ;//
 ...
 static unsigned char laser_indicate_steps = 0 ;
  ...
  if(file_type == 0)return;
		...
		if(laser_indicate_steps == 0)
	{
		...
		/* SERIAL_PROTOCOLPAIR("0 x_start=", x_start);
		 SERIAL_PROTOCOLPAIR("  x_end=", x_end);
		 SERIAL_PROTOCOLPAIR("   y_start=", y_start);
		 SERIAL_PROTOCOLLNPAIR("  y_end=", y_end);
		 */
		cvalue[0]= 0;
	    enqueue_and_echo_commands_P(PSTR("G28 X Y"));//先归零再运动到制定高度
