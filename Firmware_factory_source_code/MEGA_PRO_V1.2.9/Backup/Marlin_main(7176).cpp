...
static char laser_print_steps = 0;
    ...
    //     NEW_SERIAL_ERRORLN(gcode_LastN);
       ...
       case 0://A0 GET HOTEND TEMP 
                ...
                //              else{
		                                    ...
		                                    TFT_SERIAL_ENTER();    
                           	               if(laser_status == 1)//�رռ���
							              {    
							               WRITE(HEATER_0_PIN, 0);//��ͣʱ�رռ���
                                           Laser_print_pause = 1;
                    ...
                    //               }                       
                          ...
                          {    
                          	if(laser_status == 1)//�رռ���
							  {
							    WRITE(HEATER_0_PIN, 0);//�رռ���
								laser_status = 0;laser_print_steps = 0;
								Laser_print_pause = 0;
                         ...
                         if((!planner.movesplanned())&&(!TFTresumingflag))
                          {
                            ...
                            if(starpos!=NULL)
							 ...
							 file_type = 0;
							if(strstr(TFTstrchr_pointer,".bmp"))//�����bmpͼƬ�ļ�
							{
							  file_type = 1;
							}
	                ...
	                else if(planner.movesplanned()&&en_continue==1)//������˶���û��ɣ����յ�����Ϣ����������������ӡ
	                	{
	                	   SERIAL_PROTOCOLLN("*********open file cmd*******");
							  ...
							  *(starpos-1)='\0';
							  next_file[0]  =  0;
							  strcat( next_file , TFTstrchr_pointer + 4);  //�����ļ�
                   ...
                   if((!planner.movesplanned())&&(!TFTresumingflag))
                          {
                           ...
                           {
                              coorvalue=TFTcode_value();movespeed = movespeed/5;
...
static char temp=0;
 ...
 { 
   ...
   if(temp==2){enqueue_and_echo_commands_P(PSTR("G1 E-5"));} //����һ��
   if(temp==3){enqueue_and_echo_commands_P(PSTR("G1 Z+20 E-5")); pauseCMDsendflag=false;temp=0;} //����һ��
 }
}
/*
...
#ifdef probe_thickness
planner.buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS]- probe_thickness, destination[E_AXIS], fr_mm_s, active_extruder);
#else
...
void Laset_print_picture(void)
{
  ...
  if(Laser_printer_st.pic_vector==0)
	...
	ftemp = 2*LASER_PRINT_SPEED;//ʸ��ͼ����


	if(Laser_printer_st.pic_x_mirror==1)
		{
		  x_start = MAX_X_SIZE/2  + LASER_X_OFFSET + (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;//X��ʼ����
		  x_end   = MAX_X_SIZE/2  + LASER_X_OFFSET - (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;
		}
	else
		{
		 x_end   = MAX_X_SIZE/2  + LASER_X_OFFSET + (Laser_printer_st.pic_widht * Laser_printer_st.pic_pixel_distance)/2;//X��ʼ����
	...
	if(Laser_printer_st.pic_y_mirror==1)
		{
		  y_start=MAX_Y_SIZE/2  + LASER_Y_OFFSET + (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;//X��ʼ����
		  y_end  =MAX_Y_SIZE/2  + LASER_Y_OFFSET - (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;
		}
	else
		{
		 y_end =MAX_Y_SIZE/2  + LASER_Y_OFFSET + (Laser_printer_st.pic_hight* Laser_printer_st.pic_pixel_distance)/2;//X��ʼ����
		 y_start =MAX_Y_SIZE/2  + LASER_Y_OFFSET - (Laser_printer_st.pic_hight * Laser_printer_st.pic_pixel_distance)/2;
		}
	
	NEW_SERIAL_PROTOCOLPGM("J29");//hotend heating 
	TFT_SERIAL_ENTER();
	NEW_SERIAL_PROTOCOLPGM("J29");//hotend heating 
	TFT_SERIAL_ENTER();
...
void laser_indicate()//����ָʾ
{
 static unsigned char laser_indicate_steps = 0 ;
 static unsigned char laser_on_flag = 0 ;
  static unsigned long times = 0;
  if(times>100)
  	{
  	 times -- ;
  ...
  if(file_type == 0)return;
		...
		if(laser_indicate_steps == 0)
	{
	    ...
	    //while (planner.blocks_queued());//�ȴ��˶�ֹͣ	
	    WRITE(HEATER_0_PIN, 0);
		enqueue_and_echo_commands_P(PSTR("G28 X0 Y0"));//�ȹ������˶����ƶ��߶�
		...
		destination[Z_AXIS]= current_position[Z_AXIS] ;//��ֹZ�½�
...
//  if((AssistLeveTestflag==1)&&(commands_in_queue < BUFSIZE)){ AssistLevelTest();}
  ...
  if(file_type==1&&card.sdprinting==true)
  	{
      prepare_laser_print();//����Ҫ������������ȹ��㣬���˶���Ԥ���߶ȣ�Ȼ��ſ�ʼ������?
    }
}
