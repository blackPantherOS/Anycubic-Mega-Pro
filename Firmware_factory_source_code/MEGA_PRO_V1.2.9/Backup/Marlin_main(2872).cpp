				...
				if(i%2==0)  //偶数行 ，从头到尾扫描
				{
					  //Y=21;
						for(j=0;j<x_max;j++)
						{
								 ...
								 if(Y>MIN_GRAY_VLAUE)
								 {
										 destination[X_AXIS]=j*PIC_FIXED+80;//-((float)x_max/2-j)*PIC_FIXED+80;
										 destination[Y_AXIS]= i*PIC_FIXED+120;//((float)y_max/2-i)*PIC_FIXED+80;
				...
				else       //基数行 ,从尾到扫描
				{
						...
						for(j=0;j<x_max;j++)
						{
								 ...
								 {								 
									 ...
									 destination[X_AXIS]=(x_max-j)*PIC_FIXED+80;//((float)x_max/2-j)*PIC_FIXED+80;
									 destination[Y_AXIS]=i*PIC_FIXED+120;//((float)y_max/2-i)*PIC_FIXED+80;
									 line_to_destination(LASER_PRINT_SPEED);
									 set_current_to_destination();
									 while (planner.blocks_queued());
